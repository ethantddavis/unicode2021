...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (typeof ethereum !== 'undefined') {
      ...
      try {
        ...
        const randomSwap = new web3.eth.Contract(
          RandomSwap.abi, 
          "0x62Bb2142A967955637dD784d45aB454Fcdd3d443"
        )
  ...
  async getAccount(ethereum, web3) {
  ...
  async swap(amount) {
    if (this.state.swap !== 'undefined') {
      try {
        //console.log(amount)
        await this.state.swap.methods.convertExactEthTo(
          this.state.tokenAddresses.dai
        ).send({ 
